...
static VkQueue                  g_QueueOffscreen = VK_NULL_HANDLE;
static uint32_t                 g_QueueFamilyOffscreen = (uint32_t)-1;
static VkCommandPool			g_CommandPoolOffscreeen = VK_NULL_HANDLE;
static VkCommandBuffer			g_AllocatedCmdBufferOffscreen = VK_NULL_HANDLE;
...
static void FrameRender(ImGui_ImplVulkanH_Window* wd, ImDrawData* draw_data)
{
	...
	}
	{
		...
		auto& allocatedCommandBufferOffscreen = g_AllocatedCmdBufferOffscreen;
		if (allocatedCommandBuffers.size() > 0)
		{
			vkFreeCommandBuffers(g_Device, g_CommandPoolOffscreeen, 1, &allocatedCommandBufferOffscreen);
			allocatedCommandBuffers.clear();
		}
...
namespace Walnut
{
	...
	VkCommandBuffer Application::GetCommandBufferOffscreen(bool begin)
	{
		// Use offscreen command queue
		VkCommandPool command_pool = g_CommandPoolOffscreeen;
		...
		VkCommandBuffer& command_buffer = g_AllocatedCmdBufferOffscreen;
		auto err = vkAllocateCommandBuffers(g_Device, &cmdBufAllocateInfo, &command_buffer);
	...
	void Application::FlushCommandBufferOffscreen(VkCommandBuffer commandBuffer)
	{
		const uint64_t DEFAULT_FENCE_TIMEOUT = 100000000000;
